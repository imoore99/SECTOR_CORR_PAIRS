...
const initialPairData = sampleCurrentSignals.find(pair => pair.pair_id === 'AXP_BAC');

// Calculate days diverged
const days = countDaysDiverged(initialPairData.pair_id, sampleHistoryData);

// console.log(`${initialPairData.pair_id} has been diverged for ${days} days`);
...
// console.log(initialPairData.signal_direction)
...
// After setting initial pair data
// updateZScoreIndicator(initialPairData.z_score);
...
const initialNormalizedChart = sampleSpreadsHistory.filter(pair => pair.pair_id === 'AXP_BAC');
...
function NormalizedChart(chartData, stock1, stock2) {
    ...
    console.log("calcualted_value:", calculatedValue)
...
dropdownTag.addEventListener('change', function() {
    ...
    const chartData = sampleSpreadsHistory.filter(row => row.pair_id === selectedPairId);
